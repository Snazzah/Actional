...
type ParentNspNameMatchFn = (name: string, query: object, fn: (err: Error | null, success: boolean) => void) => void;
...
class ActionalServer extends EventEmitter {
  ...
  constructor(actionalOptions: ActionalServerOptions = {}, options?: SocketIO.ServerOptions) {
    ...
    super();
    this.server = new SocketIO.Server(options);
  ...
  private _hookParentNamespaceEvents(): void {
    this.server.on('connect', async (socket: SocketIO.Socket) => {
      ...
      if (query.rooms) await socket.join(query.rooms.split(','));
      ...
      debug(
        'socket connected %s (name=%s priority=%d rooms=%s)',
        socket.id,
        clientInfo.name,
        clientInfo.priority,
        query.rooms
  ...
  private _hookSocketEvents(socket: SocketIO.Socket): void {
    ...
    socket.on(
      'actional_sendToClient',
      // eslint-disable-next-line no-unused-vars
      async (clientId: string, eventName: string, args: unknown[] = [], callback: (data: unknown) => void) => {
        ...
        try {
          ...
          )[0] as ClientEventResponse<unknown>;
  ...
  async emitToClients(
    ...
    eventName: string,
    ...data: any[]
  ): Promise<PromiseDefinition<any[], SocketDefinition>[]> {
    const results = await allSettled(
      sockets.map((socket) =>
        definePromise(sendAndRecieve({ socket: socket.socket, timeout: this.config.ackTimeout }, eventName, ...data), {
          id: socket.socket.id,
          name: socket.name,
